...
**a)** Se utiliza la **categoría ocupacional** como variable de agrupamiento. Esta comparación es relevante porque cada modalidad de inserción laboral (patrón, cuentapropista, asalariado) implica condiciones económicas estructuralmente distintas: los patrones organizan factores de producción, los cuentapropistas operan de forma independiente y los asalariados trabajan en relación de dependencia.

**b)** Para cada grupo $i$ el parámetro de interés es la **media del ingreso mensual** ($\mu_i$): $\mu_1$ para patrones/empleadores, $\mu_2$ para cuentapropistas y $\mu_3$ para asalariados. El desvío estándar poblacional es desconocido, por lo que se aplica la distribución $t_{n_i-1}$. Los tamaños de grupo superan los `r min(ic_ingreso$n)` casos, garantizando la validez del TLC aun con la distribución asimétrica del ingreso.
