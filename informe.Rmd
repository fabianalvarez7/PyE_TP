...
**b)** Para cada grupo $i$ el parámetro de interés es la **media del ingreso mensual** ($\mu_i$): $\mu_1$ para patrones/empleadores, $\mu_2$ para cuentapropistas y $\mu_3$ para asalariados. El desvío estándar poblacional es desconocido, por lo que se aplica la distribución $t_{n_i-1}$. Los tamaños de grupo superan los `r min(ic_ingreso$n)` casos, garantizando la validez del Teorema Central del Límite aun con la distribución asimétrica del ingreso.
...
El IC de patrones/empleadores ($\hat{\mu}_1 = $ `r pesos(ic_ingreso %>% filter(Cat_ocupacional=="Patrón/empleador") %>% pull(media))`) **no se superpone** con los de los otros grupos, lo que indica que su ingreso medio es claramente superior. En cambio, los intervalos de cuentapropistas y asalariados **sí se superponen**, lo que sugiere que no existe evidencia suficiente para afirmar una diferencia significativa en el ingreso promedio entre ambos grupos con esta muestra.
